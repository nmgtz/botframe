...
const tanzaniaRegions = ["Arusha", "Dar es Salaam", "Dodoma", "Geita", "Iringa", "Kagera", "Katavi", "Kigoma", "Kilimanjaro", "Lindi", "Manyara", "Mara", "Mbeya", "Morogoro", "Mtwara", "Mwanza", "Njombe", "Pemba North", "Pemba South", "Pwani", "Rukwa", "Ruvuma", "Shinyanga", "Simiyu", "Singida", "Songwe", "Tabora", "Tanga", "Zanzibar North", "Zanzibar South and Central", "Zanzibar Urban/West"];
var physWordList = [
'bernoulli\'s theorem', 'theorem', 'physics', 'physician', 'newton', 'einstein', 'relativity', 
    ...
    'Capillary', 'Carbon Cycle', 'Carbohydrate','Chemical composition of cell', 'Cell', 'Cell Theory', 'Celebral Spinal', 
    ...
    'Ribosome', 'R-RNA (Ribosomal RNA)', 'RNA', 'Sperm', 'Species', 
    ...
    'Water Molds'
                ...
                var botWordList = [
                    'aim of chatnmg','chatnmg','nmgai', 'nmg','nurdin mohamed',
                ];
                var envirnWordList = [
    ...
    'hello', 'hi', 'hey', 'yo', 'greetings', 'salutations', 'howdy', 'hola', 'bonjour', 'ciao', 'hallo', 'namaste','goodbye','thanks'
                ...
                var swahiliWordList = [
                    ...
                    'chimbuko','jamani','lugha', 'msamiati','misamiati','neno',"habari", "karibu", "asante", "samahani", "tafadhali", "ndiyo", "hapana", 
    "pole", "rafiki", "chakula", "maji", "ndizi", "mtoto", "mama", "baba", 
    "nina", "sijui", "nasema", "ninapenda", "kutoka", "safari", "mchezo", 
    "tutaonana", "nzuri", "kesho", "leo", "jana", "ndugu", "daktari", 
    "shule", "nyumbani", "sawa", "kwaheri", "mwalimu", "kazi", "mji", 
    "barabara", "gari", "moto", "nguo", "pesa", "dukani", "kijiji", "mchana", 
    "usiku", "asubuhi", "jioni", "muda", "siku", "wakati", "mara", "polepole", 
    "haraka", "moja", "mbili", "tatu", "nne", "tano", "sita", "saba", "nane", 
    "tisa", "kumi", "kwa", "na", "lakini", "au", "hivi", "kama", "kuna", 
    "kuchukua", "kuleta", "kuona", "kuenda", "kurudi", "kulala", "kuamka", 
    "kupenda", "kufanya", "kusoma", "kuandika", "kujua", "kuelewa", "kusikia", 
    "kuuliza", "kujibu", "kusema", "kufikiri", "kuwaza", "kukimbia", "kutembea", 
    "kucheza", "kufurahia", "kulia", "kuimba", "kushangilia", "kupika", "kula", 
    "kunywa", "kuogelea", "kujifunza", "kufundisha", "kutafuta", "kupata"
                ];
                var geoWordList = [
...
["RNA", "RNA (Ribonucleiic Acidd)", "RNA (Ribonuclei Acid)", "RNN (Ribonucleic Acid)", "RNAA (Ribonucleic Acid)", "RNA (Ribonucleiic Acid)","rna"],
...
function normalizePhrase(phrase) {
    let cleanedPhrase = removeSomeDt(phrase).toLowerCase();
    let originalPhrase = cleanedPhrase; 

    for (let synonyms of synonymsArray) {
        const mainPhrase = synonyms[0].toLowerCase();
        for (let i = 1; i < synonyms.length; i++) {
            const synonym = synonyms[i].toLowerCase();
            const regex = new RegExp(`\\b${synonym.replace(/\s+/g, '\\s+')}\\b`, 'gi');
            cleanedPhrase = cleanedPhrase.replace(regex, mainPhrase);
        }
    }

  
    return cleanedPhrase === originalPhrase ? phrase : cleanedPhrase;
}

function isMultiWord(phrase) {
    return phrase.trim().includes(' ');
}
...
function formatDescription(description) {
    ...
    let restOfDescription = lines.join('<br><br>');
    restOfDescription = restOfDescription.replace(/(\.\s*)(?=[A-Z])/g, '$1<br>');
    const formattedDescription = (firstLine + '<br>' + restOfDescription)
            .replace(/[\/\-@]/g, '') 
            .replace(/\s{2,}/g, ' '); 
...
function phraseWords(phrase) {
    ...
    var normalizedPhrase = normalizePhrase(freshWords); 
    var words = normalizedPhrase.split(/\s+/);
    var botResponse = "";

    var dissatisfactionResponse = handleUserDissatisfaction(normalizedPhrase);
    if (dissatisfactionResponse) {
        botResponse = dissatisfactionResponse;
    } else {
        function findMatch(phraseList) {
            var multiWordPhrases = phraseList.filter(phrase => isMultiWord(phrase));
            var singleWordPhrases = phraseList.filter(phrase => !isMultiWord(phrase));

            var match = multiWordPhrases.find(phrase => {
                const regex = new RegExp(`\\b${phrase.toLowerCase().replace(/\s+/g, '\\s+')}\\b`, 'gi');
                return regex.test(normalizedPhrase);
            });
            if (!match) {
                match = singleWordPhrases.find(phrase => {
        ...
        var matchedStatementGreeting = findMatch(statementGreetings);
        var matchedThankingStatement = findMatch(thankingStatements);
        var botWordsQns = findMatch(botDetailsWords);
        var matchedOtherPhrases = findMatch(otherPhasesList);
        var matchedSciencePhrases = findMatch(scienceWordList);
        var matchedGeoWord = findMatch(geoWordList);
        var matchedCodeWords = findMatch(codeWordList);
        var matchedPhysWord = findMatch(physWordList);
        var matchedBiosWord = findMatch(biosWordList);
        var matchedBotWord = findMatch(botWordList);
        var matchedTanzania = findMatch(tanzaniaRegions);
        if (matchedStatementGreeting) {
            botResponse = statementGreetingsResponse(matchedStatementGreeting);
        }else if (matchedTanzania) {
            botResponse = formatDescription(tanzaniaDescribes(matchedTanzania));
